...
object FullResolvedBestMention {
  ...
  def apply(target: Entity, bestEntity: Entity, candidateCount: Double) = FullResolvedBestMentionImpl(target, bestEntity, candidateCount)
}
...
object LinkResolvedBestMention {
  ...
  def guessType(fb: FreeBaseLink): EntityType = {
    val lcTypes = fb.types.map(_.toLowerCase)
    if (lcTypes.exists(t => locRegex.findFirstIn(t).isDefined)) Location
    else if (lcTypes.exists(t => orgRegex.findFirstIn(t).isDefined)) Organization
    else if (lcTypes.exists(t => perRegex.findFirstIn(t).isDefined)) Person
    else Other
  }
}
